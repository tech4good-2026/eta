  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col justify-between px-6 py-6 overflow-y-auto custom-scrollbar bg-slate-50" id="view-arrival-screen">
      <div className="text-center pt-3">
        <div className="w-[66px] h-[66px] rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto mb-4.5 border border-emerald-100 shadow-sm">
          <Check className="w-[32px] h-[32px]" />
        </div>
        <h1 className="text-[21.5px] font-bold text-slate-900 mb-1">안전하게 완주했습니다!</h1>
      ...
      <div className="grid grid-cols-3 gap-2 py-4">
        <div className="bg-white border border-slate-200 p-3 rounded-2xl text-center shadow-sm">
          <span className="block text-xs text-slate-400 font-bold mb-0.5">실제 완주 소요</span>
          <b className="text-[16px] font-mono text-blue-600">{route.personal}분</b>
        </div>
        <div className="bg-white border border-slate-200 p-3 rounded-2xl text-center shadow-sm">
          <span className="block text-xs text-slate-400 font-bold mb-0.5">환승 수단</span>
          <b className="text-[16px] font-mono text-blue-600">{route.transfers}회</b>
        </div>
        <div className="bg-white border border-slate-200 p-3 rounded-2xl text-center shadow-sm">
          <span className="block text-xs text-slate-400 font-bold mb-0.5">지불 요금</span>
      ...
      <div className="bg-white border border-slate-200 rounded-[22px] p-4.5 shadow-sm space-y-3">
        ...
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 grid grid-cols-2 gap-2 text-center text-slate-700 font-bold">
          <div className="border-r border-slate-200 py-0.5">
            <span className="block text-xs text-slate-400">이전 속도 산정</span>
            <span className="text-base font-mono text-slate-800">{previousSpeed} m/s</span>
          </div>
          <div className="py-0.5">
            <span className="block text-xs text-slate-400">완료 후 속도</span>
            <span className="text-base font-mono text-blue-600">{speed} m/s</span>
          </div>
        </div>

        <p className="text-xs text-slate-400 font-bold">표본: {samples}회</p>
        ...
        {baseSpeed != null && maxSpeed != null && (
          <div className="bg-blue-50/60 p-2.5 rounded-xl border border-blue-100 grid grid-cols-2 gap-2 text-center text-slate-700 font-bold">
            <div className="border-r border-blue-100 py-0.5">
              <span className="block text-xs text-slate-400">센서 기본속도</span>
              <span className="text-base font-mono text-slate-800">{baseSpeed} m/s</span>
            </div>
            <div className="py-0.5">
              <span className="block text-xs text-slate-400">센서 최고속도</span>
              <span className="text-base font-mono text-blue-600">{maxSpeed} m/s</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 pt-4">
        <div className="space-y-2">
          ...
          <button
            onClick={onLogout}
            className="w-full py-3 bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 rounded-full font-bold text-base flex items-center justify-center gap-1.5 cursor-pointer"
          >
