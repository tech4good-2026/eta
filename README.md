...
| 역할 영역 | 담당 내용 |
|---|---|
| 기획·리서치 | 문제 정의, 교통약자 사용자 시나리오, 공공데이터·API 조사 |
| UI·UX·발표 | 모바일 화면 설계, 접근성 상태 표현, 발표 자료와 시연 시나리오 제작 |
| 프론트엔드 | React 화면, 카카오맵·장소 검색, 경로·안내·재탐색 UI |
| 백엔드·API | FastAPI, TMAP·서울 공공데이터 연동, 경로·안내 세션 관리 |
| AI·데이터 | 개인별 보행속도 보정, 접근성 판정, 지형·시설 데이터 처리 |
| QA·시연 | 대표 경로 검증, 외부 API 장애 대응, 발표 환경 구성 |

각 역할은 기능 단위로 교차 협업했습니다.
