...
> 현재 백엔드 수직 슬라이스는 프로필 조회·수정, 경로 검색·조회, 안내 시작·위치 처리·재탐색·완료를 구현합니다. 인증은 `Bearer demo-token`을 사용합니다. 회원가입·JWT·SQLite와 백엔드 장소 검색은 후속 범위이며, 현재 장소 검색은 카카오 브라우저 SDK가 담당합니다.
...
프론트는 전경에서 최대 5초에 한 번 전송합니다. 서버는 정확도·시간 간격·속도 범위를 만족하고 이전·현재 좌표가 모두 도보 경로 근처인 표본만 속도 학습에 사용합니다. 경로 이탈, 대중교통 구간, 재탐색, 안내 완료 시 직전 좌표는 초기화하며 완료 후 원본 위치는 남기지 않습니다. 사용자 확인 전에는 `routeRevision`과 경로를 변경하지 않습니다.
...
유효한 도보 표본이 3개 이상이면 세션 중앙값과 기존 평상시 속도를 누적 표본 수로 가중해 `walkingSpeedMps`와 `baseSpeedMps`를 갱신합니다. `walkingSpeedSampleCount`는 기존·신규 표본 수의 합이며 `maxSpeedMps`는 기존 최고값과 새 유효 최고값 중 큰 값입니다. 유효 표본이 부족하면 `walkingSpeedUpdated: false`와 기존 속도를 반환합니다. 같은 세션을 다시 완료하면 `409 SESSION_ALREADY_COMPLETED`입니다.
