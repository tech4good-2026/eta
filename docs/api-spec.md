...
서버가 보유한 임시 집계를 이용해 프로필 속도를 갱신합니다. 현재 수직 슬라이스는 실제 보행 속도 집계를 생성하지 않으므로 `walkingSpeedUpdated: false`와 기존 속도를 반환합니다. 같은 세션을 다시 완료하면 `409 SESSION_ALREADY_COMPLETED`입니다.
