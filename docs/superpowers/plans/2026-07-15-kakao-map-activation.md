...
**Goal:** 카카오 Developers 앱을 지도 사용 가능한 상태로 만들고 JavaScript 키를 로컬 프론트엔드에 연결해 장소 검색과 지도 렌더링을 검증한다.

**Architecture:** 카카오 Developers의 `Tech4Good Navigation` 앱에 등록용 아이콘을 추가하고 개인 개발자 비즈 앱 전환 및 카카오맵 추가 기능 신청을 진행한다. 승인된 JavaScript 키는 Git에서 제외되는 `frontend/.env`에만 저장하며, React/Vite 앱은 기존 Kakao Maps SDK 로더와 Places 서비스를 그대로 사용한다.
...
- [ ] **Step 2: 프로젝트 자산으로 복사하고 128×128px로 변환**
...
- Produces: 카카오맵 권한을 보유한 `Tech4Good Navigation` 앱
...
- [ ] **Step 3: 비즈니스 정보 심사 신청**

`앱 설정 > 앱 > 추가 기능 신청`에서 신청 자격에 동의하고 `비즈니스 정보` 심사를 신청한다. 서비스 설명은 다음 문구를 사용한다.
...
Expected: 비즈니스 정보 심사 상태가 `심사 중` 또는 `승인`으로 표시.

- [ ] **Step 4: 카카오맵 추가 기능 신청 및 활성화**

비즈니스 정보 승인 후 `추가 기능 > 카카오맵 > 신청`을 제출하고, 승인되면 `제품 설정 > 카카오맵 > 사용 설정`을 `ON`으로 변경한다.

Expected: 카카오맵 사용 설정 `ON`.
...
**Interfaces:**
- Consumes: 카카오맵 권한이 활성화된 앱의 JavaScript 플랫폼 키
- Produces: Vite 런타임에서 로드 가능한 `VITE_KAKAO_MAP_APP_KEY`

- [ ] **Step 1: Web 플랫폼과 도메인 등록**

`앱 설정 > 앱 > 플랫폼 키`에서 JavaScript 키를 선택하고 Web 플랫폼에 `http://localhost:5173`을 등록한다. 배포 도메인은 실제 URL이 결정되기 전에는 추가하지 않는다.
...
- Produces: 브라우저에서 검증된 로그인→장소 검색→대중교통 경로 흐름
...
Expected: 백엔드 44개 테스트, 프론트 13개 테스트, Ruff, TypeScript, Vite build 모두 통과.
...
Expected: 지도와 검색 결과 표시, `/api/v1/routes/search` 200, 브라우저 콘솔 오류 없음.

- [ ] **Step 4: 종료와 Git 상태 확인**

```bash
git status --short --branch
```

Expected: `frontend/.env`와 공급자 키가 Git 변경 목록에 없고, 의도한 아이콘·문서 커밋만 존재.
